...
import SucessUnsuccessContainer from "./SucessUnsuccessContainer";
...
const UploadComponent = ({
  ...
  const handleUploadButtonClick = () => {
    handleTrackDetailsUpdate(
      { id: "mediaType", value: ENUMS.MEDIA_TYPE_FILEUPLOAD },
      index
    );
  ...
  const handleSelectedFeedback = useCallback(
    (args) => {
  ...
  const handleOnSelectContainer = useCallback((e, value) => {
    ...
    <div className="upload-main-container">
      <div className="upload-header-container">
        <div className="app-name-container">
          <span className="app-display-name">{content.UPLOAD}</span>
          <br />
        </div>
        <div className="header-icon-container">
          <MoneyBag />
          <div className="header-icon-text-container">
            <p className="coin-number">{userDetails.balance}</p>
            <p className="coin-text">{content.COIN}</p>
          </div>
          <FireIcon />
          <div className="header-icon-text-container">
            <p className="coin-number">Lv.0</p>
            <p className="coin-text">{content.RATER}</p>
          </div>
        </div>
      </div>
      <div className="buttons-main-component">
        <Tabs
          ...
          onChange={handleOnSelectContainer}
        >
          <Tab label="Upload" />
          <Tab label="Results" />
        </Tabs>
        {/* <Button
          className={cx(
            "unSelected-button",
            selectedContainer === 0 && "selected-button"
          )}
          buttonText="Upload"
          onClick={() => handleOnSelectContainer(0)}
        ></Button>
        <Button
          className={cx(
            "unSelected-button",
      ...
      {selectedContainer === 0 && (
        <section>
          <div className="submit-music-heading-container">
            <span className="submit-music-queue-txt">
              {content.SUBMIT_MUSIC_QUEUE}
            </span>
            {/* <Help /> */}
          </div>
          ...
          <div
            className="browse-file-container"
            onClick={handleUploadButtonClick}
          >
            {tracks.fileUpload && (
              <p className="file-name-text">{tracks.fileUpload.name}</p>
            )}
            <div className="browse-file-text-container">
              <span className="browse-text">{content.BROWSE}</span>{" "}
              <span className="file-text">{content.FILE}</span>
            </div>
          </div>
          <input
            accept=".mp3"
            ...
            <Elements>
              <section>
                <div className="payment-option-container">
                  <span className="payment-option-header-text">
      ...
      {selectedContainer === 1 && <ResultContainer />}
      {isProcessing && (
        <DialogBox bodyComponent={<OrderProcessing />} hideClose />
      )}
      {isSuccess && (
        <DialogBox
          bodyComponent={
      ...
      {isUnSuccess && (
        <DialogBox
          bodyComponent={
            <SucessUnsuccessContainer onCloseSucess={onCloseUnSucess} />
          }
          hideClose
        />
      )}
